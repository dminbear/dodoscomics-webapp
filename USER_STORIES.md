...
1.
  * **EPIC**
    * Access
  * **Role-Goal-Benefit**
    * As a new viewer or new contributor I want to be able to create an account so that I can use the web-application.
  * **Notes**
  ...
  * **Definitions of Done (Test)**
    * A new viewer or new contributor should be able to successfully create an account if they provide a valid inputs for the following fields (* indicates a required field):
      * *email.
      * *username.
      * *password.    
      * *choose between "Viewier" or "Contributor".    
      * upload an avatar.
    * A user knows that they have successfully created an account if after clicking "Create Account" they are logged in succesfully and redirected to their profile page which displays the personal information that they have provided.
    * A new viewer or new contributor will be given an appropriate error message and will fail to create an account if they:    
  ...
  * **Tasks**
    ...
    * Redirect a succesful registration to the profile page after logging the user in.
  * **Effort Estimate**  
    * 6 hours
...
2.
  * **EPIC**
    * Access
  * **Role-Goal-Benefit**
    *As a current viewer or current collaborator I want to be able to login with my previously registered credentials so that I can access the site functionality depending on my account privileges.
  * **Notes**
    * Login means being able to access account privileges such as:
      ...
      * changing avatar.
      * submitting comments.
      * in the case of a contributor, the ability to add and edit comic strips.
  * **Definitions of Done (Test)**
    * Viewer or collaborator will successfully log in if after typing in their valid username and password and clicking the "login" box. They will then see their profile page.
  * **Tasks**
    ...
    * Implement functionality to verify that the correct username and password was supplied. If it was then redirect the user to their profile page after logging them in. If not then display an error message.
  * **Effort Estimate**
    * 4 hours
...
3.
  ...
  * **Notes**
    * Should be able to access following things (the account privileges):
      * change password.
  * **Definitions of Done (Test)**
    * Current logged in viewer or collaborators should be able to change their password in the "Account" page by putting in a new password in the "Change Password" box and clicking "Update".
    * Successful change if viewer or collaborator is able to log in to their account with their updated password.
  * **Tasks**
    * Create a link that will lead to the "Account" page.
    * Create a form for showing the profile info.
    * Create box for "Change Password".
    * Update database with the changed password associated with the viewer or collaborator when password is succesfully changed.
    * Implement a password checking algorithm that checks for the strength of the password by making sure that at least 6 characters were used.
    * Show an appropriate error message to the user if the new password supplied was either a) invalid or b) unsuccesfully changed.
  * **Effort Estimate**
    * 4 hours
...
4.
  * **EPIC**
    * Storage
  * **Role-Goal-Benefit**
    * As a viewer or contributor, I want to be able to change my avatar image when I am logged in so it can be displayed on my profile and in my public comments.
  * **Notes**
    * Size should be constrained (300px by 300px).
    * Supports JPEG, GIF, and PNG.
  * **Definitions of Done (Test)**
    * Users should be able to upload an avatar image from their "Account" page and selecting a picture from their files to their profile in 
      * Profile page displays the correct avatar picture after upload.
      * Upload fails if image size is greater than limit or if it is not a supported image type. User will get an appropriate error message.
    * When they comment on comcic strips, a smaller version of their avatar is displayed next to their username.
  * **Tasks**
    * Enable a separate image upload for avatars (in "Account" page options).
...
5.
  ...
  * **Definitions of Done (Test)**
    * A current user account can add a comic strip (by clicking a "Save to Favourites"  button near the strip) to their favorites. If they navigate to their profile they can then find their favorites tab with that saved strip link on it.
  * **Tasks**
    * Verify whether the user is logged in (if not, lead to log-in page).
    * If the selected comic strip is already in the favorites, show "Remove from Favorites" button instead of add to favorites.
    * If the selected comic strip is not in the favorites, show 'Add to Favorites'.
  * **Effort Estimate**
    * 3 hours
...
7.
  ...
  * **Notes**
    * Comments will be below the comic strip itself.
  ...
  * **Definitions of Done (Test)**
    ...
    * The comment will be viewable to all other users on the page.
  * **Tasks**
    * Verify whether the user is logged in (if not, lead to log-in page).
    * Create the form for user to type a comment.
    * Save the comment submitted in the database.
    * Have all the comments in the database for a given comic display on the comic page in the comments section on the front-end.
  * **Effort Estimate**
    * 3 hours
8.
  * **EPIC**
  ...
  * **Definitions of Done (Test)**
    * Typing into the search engine will give out relevant results on a separate page for the user to browse.
  * **Tasks**
...
9.
  * **EPIC**
    * Editing
  * **Role-Goal-Benefit**
    * As a contributor, I want to be able to give my comic a title and description when I upload it, so that others can read my comments about the strip.
  * **Notes**
    * Title is shown above the strip, and in larger font than author comments.
    * Description is below the strip, but above the user comments.
  ...
  * **Definitions of Done (Test)**
    ...
    * Once uploaded and the information entered, title will be displayed above the comic strip and the description will be displayed below.
  * **Tasks**
    * Enable text entry when uploading comics.
    * The entry will take in two different inputs, one for title and one for comments, which are entered in separate input boxes.
  * **Effort Estimate**
    * 4 hours
...
11.
  * **EPIC**
    * Editing
  * **Role-Goal-Benefit**
    * As a contributor, I want to be able to edit the title and description of my comic so that I can add or change my thoughts.
  * **Notes**
    * Can also completely delete the title and author comments so that the field is empty after it has already been set.
  * **Definitions of Done (Test)**
    * There should be an edit button next to the comic strip where the user can click and then be led to a page that allows them to edit and delete the comic title and description.
  * **Tasks**
    * Create an edit function that is activated by clicking on the "Edit" link.
    * When this link is clicked, the user can change what is in their comic title or description.
  * **Effort Estimate**
    * 4 hours
...
12.
  ...
  * **Definitions of Done (Test)**
    * Contributors can upload images for comics by adding each cell through an image upload. Then they will be displayed on the comic page where the contributor and the user can see.
  ...
  * **Tasks**
    ...
    * Display the image on the webpage after uploaded.
  * **Effort Estimate**
    * 5 hours
...
13.
  ...
  * **Definitions of Done (Test)**
    * In the login page, there will be a "forgot password" button that brings the user to the reset password page where they will enter their email. If the email matches an email in the database then they will recieve an email with a link to reset the password.
    * A appropriate confirmation message will be shown after an email has been typed in.
    * The user can use the sent link to go to a page where it lets you type in a new password.
  * **Tasks**
    * Implement the forgot password button.
    * It will open a new page that lets you type in an email.
    * The email typed in will be searched for within the database and a forgot password link will be sent if the email is found in the database. If not, no email will be sent.
    * Send the email with a reset password link associated with the account that allows the user to change their password by following the link in the email.
    * Create a reset password page that lets the user change the password of an account.
